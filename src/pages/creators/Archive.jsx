...
function CreatorsArchive() {
  ...
  const fetchItem = async (address, category) => {
    ...
    if (category === "currentBid") {
      ...
      itemIds = (ownedTokens || []).filter(
        (item) =>
          item?.contract?.id?.toLowerCase() == getAFCNFTAddress()?.toLowerCase()
      );

      console.log("owneditems:", itemIds, ownedTokens);
      ...
      //         console.log("resowneditems:", responses[i]?.data);
    ...
    axios
      ...
      .then((response) => {
        let temp = items;
        temp[category] = response.data.items || [];

        // temp[category] = (response.data.items || []).filter(
        //   (item) => !item.isSold
        // );
        setItems(temp);
        setRefresh(Date.now());
  ...
  useEffect(() => {
    if (
      account &&
      (account.toLowerCase() ===
        "0x853c2316DdcB8b615b31419A4a29e8a1079d80c0".toLowerCase() ||
        account.toLowerCase() === "0x2d97a45d32669bc5a043ec8d33c40ff7858d3607")
    ) {
      setIsAdmin(true);
    } else {
      // setIsAdmin(false)
            ...
            <div className="w-full max-w-[1400px] m-auto relative z-20 mb-0">
              <div className="flex flex-wrap items-center head">
                ...
                <div className="my-4 title">
                  <h2 className="text-[20px] sm:text-[25px] md:text-[28px] font-bold text-white mb-1">
                    {user?.fullname}
                  </h2>
                  <p className="flex items-center text-base text-white">
                    @{user?.username}
                    {didVerify && (
                      <span className="ml-1 w-[16px]">
                        <svg
              ...
              <div className="flex flex-wrap items-center justify-between archive-banner-bottom">
                ...
                <div className="flex flex-wrap items-center justify-start right sm:justify-end">
                  <h3 className="my-3 mr-4 text-sm font-bold text-white md:text-base lg:text-lg md:mr-5 lg:mr-8">
                    <span className="mr-1">{user?.follower}</span>
                    Followers
                  </h3>
                  <h3 className="my-3 mr-4 text-sm font-bold text-white md:text-base lg:text-lg md:mr-5 lg:mr-8">
                    <span className="mr-1">{user?.following}</span>
                    Followings
                  </h3>
                  ...
                  <div className="share">
                    <button
                      ...
                      type="button"
                      className="bg-white text-black min-w-[90px] rounded-3xl cursor-pointer text-base h-[35px] leading-[35px]"
                    >
                      Share
                    </button>
                  </div>
                  ...
                  {isAdmin && (
                    <div className="share">
                      <button
                        onClick={() => verify()}
                        type="button"
                        className="bg-white text-black min-w-[90px]  ml-3 rounded-3xl cursor-pointer text-base h-[35px] leading-[35px]"
                      >
                        {didVerify ? "Disprove" : "Verify"}
                      </button>
                    </div>
                  )}
                </div>
              ...
              <div className="creators-archive-page-inner">
                <nav className="mt-5 archive-nav">
                  <ul className="flex flex-wrap items-center justify-center">
                    <li>
                      <button
                        ...
                        className={classNames(
                          "profile_tab_btn border-transparent",
                          active === "created" && "border-rose-700"
                        )}
                      >
                        Created
                      </button>
                    ...
                    <li>
                      <button
                        onClick={() => setActive("currentBid")}
                        className={classNames(
                          "profile_tab_btn border-transparent",
                          active === "currentBid" && "border-rose-700"
                        )}
                      >
                        Current Bid
                      </button>
                ...
                >
                  <div
                    className={`left-filter overflow-hidden bg-white rounded-3xl lg:sticky fixed z-30 top-16 lg:block lg:left-0 lg:self-start transition ease-in ${
                      filterActive
                  ...
                  <div
                    className={`all-nfts-wrapper grid transition ease-in gap-7 ${
                      filterActive
                        ? "filter-active w-full lg:w-[calc(100%-300px)] lg:pl-8"
                        : "w-full"
                    } ${
                      layout
                        ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 layout-active"
                        : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5"
                    }`}
                  >
                    {active === "created" && (
                      <>
                        {items?.created?.map((nft, index) => {
                          return <NFTItemsCard key={index} postDetails={nft} />;
                        })}
                      </>
                    )}
                    ...
                    {active === "currentBid" && (
                      <>
                        {items?.currentBid?.map((nft, index) => {
                          return <NFTItemsCard key={index} postDetails={nft} />;
                        })}
                      </>
                    )}
...
export default CreatorsArchive;
